...
public class GameBlueming extends GameState
   ...
   public void init() {
      ...
      failure = false;      //실패여부 기록
      
      // beat
      beatSound = new Music("beat.mp3", false);      //무한 반복으로 해야함
   ...
   public void setBeat() {      //떨어지는 beat 하나하나 설정
      ...
      beats = new Beat[] {
            ...
            new Beat(startTime + gap * 13, true, 1), // 이거 할때 coin인 경우는 position 앞에 true만 써주면 됩니다.
            ...
            new Beat(startTime + gap * 21, true, 5), // 사과 톡톡톡 
   ...
   // 순차탐색을 돌며 해당 노트 판단 
   public void judge(int input)      //input 으로 position 들어옴
   {
	   String clear="";
   ...
   public void run() {
      ...
      while(gameMusic.getTime() <= 99999 && failure == false && !game.isInterrupted()) {      //노래시간남음 && 받기성공 -> 게임 계속 진행
